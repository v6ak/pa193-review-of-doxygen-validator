...
* It is good that the tokenizer always finishes in a linear time with respect to the input length.
...
## Parser
* As notes in tokenizer, tt assumes that it has input with `\` escape sequences filtered out by the tokenizer.
